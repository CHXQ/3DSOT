...
class NuscenceTraceDataset(Dataset):
    ...
    def get_data_info(self, index):
        ...
        for i, pc_path in enumerate(pc_paths):
            ...
            # assert crop_pc.nbr_points() > 10, 'not enough search points'
...
if __name__ == '__main__':
    ...
    #     frame_num = batch_data['frame_num'][0]
    #     if (frame_num == 0):
    #         num += 1
    # print(num)
    # print('load data')
    # for batch_data in tqdm(data_loader):
    #     ranges.append(batch_data['center_range'].numpy().squeeze())
    # print(len(ranges))
    # ranges = np.array(ranges)
    # range_min = ranges[:,0]
    # range_max = ranges[:,1]
    # print(np.min(range_min, axis=0))
    # print(np.max(range_max, axis=0))
    # print(np.mean(range_max, axis=0))
    # num = 0
    loss = []
    for batch_data in tqdm(data_loader):

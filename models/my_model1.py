...
class MyModel(my_base_model.BaseModel):
    def __init__(self, config, **kwargs):
        ...
        self.point_mlp = nn.Sequential(nn.Linear(256, 128),
                                        ...
                                        nn.BatchNorm1d(128),
                                        nn.ReLU(),
                                        nn.Linear(128, self.T * 4))
        self.box_mlp = nn.Sequential(nn.Linear(128, 128),
                                        nn.BatchNorm1d(128),
                                        nn.ReLU(),
                                        nn.Linear(128, 128),
                                        nn.BatchNorm1d(128),
                                        nn.ReLU(),
                                        nn.Linear(128, self.T * 4))
        self.mlp = nn.Sequential(nn.Linear(384, 128),
                                        nn.BatchNorm1d(128),
                                        nn.ReLU(),
                                        nn.Linear(128, 128),
                                        nn.BatchNorm1d(128),
    ...
    def forward(self, input_dict):
        ...
        point_feature = self.pc_pointnet(mask_points)
        
        x2 = input_dict["track_bbox"].transpose(1,2).cuda()
        bbox_feature = self.bbox_pointnet(x2)
        
        joint_featue = torch.cat((point_feature, bbox_feature), 1)
        
        point_output_offset = self.point_mlp(point_feature)
        bbox_output_offset = self.box_mlp(bbox_feature)
        joint_output_offset = self.mlp(joint_featue)
        
        track_bbox = input_dict['track_bbox'].view((-1, 5))[:, :4]
        point_output_offset = point_output_offset.view((-1, 4))
        bbox_output_offset = bbox_output_offset.view((-1, 4))
        joint_output_offset = joint_output_offset.view((-1, 4))
        point_output_offset[:, 3] = 0
        bbox_output_offset[:, 3] = 0
        joint_output_offset[:, 3] = 0
        point_output = track_bbox + point_output_offset
        bbox_output = track_bbox + bbox_output_offset
        joint_output = track_bbox + joint_output_offset
        output_dict['point_estimation_boxes'] = point_output
        output_dict['bbox_estimation_boxes'] = bbox_output
        output_dict['joint_estimation_boxes'] = joint_output
    ...
    def compute_loss(self, data, output):
        ...
        point_estimation_boxes = output['point_estimation_boxes'][:, :3]
        bbox_estimation_boxes = output['bbox_estimation_boxes'][:, :3]
        joint_estimation_boxes = output['joint_estimation_boxes'][:, :3]
        with torch.no_grad():
        ...
        point_estimation_boxes = point_estimation_boxes * bbox_mask
        bbox_estimation_boxes = bbox_estimation_boxes * bbox_mask
        joint_estimation_boxes = joint_estimation_boxes * bbox_mask
        center_label = center_label * bbox_mask
        bbox_mask = bbox_mask.squeeze()
        point_estimation_boxes = point_estimation_boxes[bbox_mask>0]
        bbox_estimation_boxes = bbox_estimation_boxes[bbox_mask>0]
        joint_estimation_boxes = joint_estimation_boxes[bbox_mask>0]
        center_label = center_label[bbox_mask>0]
        point_loss_center = F.smooth_l1_loss(point_estimation_boxes, center_label)
        bbox_loss_center = F.smooth_l1_loss(bbox_estimation_boxes, center_label)
        joint_loss_center = F.smooth_l1_loss(joint_estimation_boxes, center_label)
        loss_seg = F.cross_entropy(seg_logits, seg_label, weight=torch.tensor([0.5, 2.0]).cuda())
        loss_total = 0.1 * loss_seg + 0.3 * point_loss_center + 0.3 * bbox_loss_center + 0.4 * joint_loss_center
        ...
        loss_dict["point_loss_center"] = point_loss_center
        loss_dict["bbox_loss_center"] = bbox_loss_center
        loss_dict["joint_loss_center"] = joint_loss_center
    ...
    def evaluate_one_sequence(self, sequence):
        ...
        estimation_box = 0.3 * output['point_estimation_boxes'] + 0.3 * output['bbox_estimation_boxes'] \
            + 0.4 * output['joint_estimation_boxes']

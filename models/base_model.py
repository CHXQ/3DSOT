...
class BaseModel(pl.LightningModule):
    ...
    def pre_build_input_dict(self, sequence, frame_id, results_bbs, **kwargs):
        raise NotImplementedError
    ...
    def extract_all_sequence(self, sequence, track):
        """
        ...
        for frame_id in range(len(sequence)):  # tracklet
            this_bb = track[frame_id]
            bbox_list.append(sequence[frame_id]['3d_bbox'])
            meta_list.append(sequence[frame_id]['meta'])
            results_bbs = [this_bb]
            for i in range(frame_id - 1, -1, -1):
                # construct input dict
                data_dict, ref_bb = self.pre_build_input_dict(sequence, i, results_bbs)
                # run the tracker
                candidate_box = self.evaluate_one_sample(data_dict, ref_box=ref_bb)
                results_bbs.append(candidate_box)
            results_bbs.reverse()
            for i in range(frame_id + 1, len(sequence)):
                # construct input dict
                data_dict, ref_bb = self.build_input_dict(sequence, i, results_bbs)
                # run the tracker
                candidate_box = self.evaluate_one_sample(data_dict, ref_box=ref_bb)
                results_bbs.append(candidate_box)
            results_list.append(results_bbs)
        output = {'bbox_gt': bbox_list, 'bbox_predict_list': results_list, 'meta': meta_list}
        return output
    ...
    def evaluate_one_sequence(self, sequence):
        ...
        for frame_id in range(len(sequence)):  # tracklet
            ...
            bbox_list.append(this_bb)
            meta_list.append(sequence[frame_id]['meta'])
            if frame_id == 0:
                # the first frame
                results_bbs.append(this_bb)
                # debug
                pc = sequence[frame_id]["pc"]
                num_points_in_prev_box = geometry_utils.points_in_box(this_bb, pc.points).sum()
                self.num_points.append(num_points_in_prev_box)
        ...
        if not self.extract_flag:
            output = {'bbox_gt': bbox_list, 'bbox_predict': results_bbs, 'meta': meta_list}
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.extract_flag:
            output = self.extract_all_sequence(sequence, result_bbs)
            self.result_lists.append(output)
        return result_bbs

    def test_epoch_end(self, outputs):
        np.save('val_num_points.npy', self.num_points)
        # if self.extract_flag:
        #     output_save_path = os.path.join(getattr(self.config, 'save_path', './'), getattr(self.config, 'test_split', '_')+ \
        #                                 '_' + getattr(self.config, 'category_name', '_') + '.dat')
        #     # output_save_path = f"output_car.dat"
        #     with open(output_save_path, 'wb') as f:
        #         print(f'saving loaded data to {output_save_path}')
        #         pickle.dump(self.result_lists, f, 0)
        #     print('write finish')
        # else:
        #     output_save_path = os.path.join(getattr(self.config, 'save_path', './'), getattr(self.config, 'test_split', '_')+ \
        #                                     '_' + getattr(self.config, 'category_name', '_') + '.dat')
...
class MotionBaseModel(BaseModel):
    ...
    def pre_build_input_dict(self, sequence, frame_id, results_bbs):
        prev_frame = sequence[frame_id + 1]
        this_frame = sequence[frame_id]
        prev_pc = prev_frame['pc']
        this_pc = this_frame['pc']

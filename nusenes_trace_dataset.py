...
class NuscenceTraceDataset(Dataset):
    ...
    def get_data_info(self, index):
        ...
        for i, pc_path in enumerate(pc_paths):
            ...
            w, l, h = gt_track[i].wlh
            gt_delta_center = (gt_track[i].center - track[i].center) / np.array((l, w, h)) * np.array((1, 1, 2))
            gt_delta_theta = theta_gt - theta
            gt_delta = np.append(gt_delta_center, gt_delta_theta).astype('float32')
            track_gt_delta.append(gt_delta)
        ...
        track_gt_delta = np.array(track_gt_delta)
        track_bbox = np.array(track_bbox)
        track_gt_delta = np.pad(track_gt_delta, ((0, self.max_frame_num - frame_num), (0, 0)), 'constant', constant_values = (0, 0))
...
if __name__ == '__main__':
    ...
    ann_file = os.path.join(data_root, 'nuscenes_track_car_train.pkl')
    ...
    for batch_data in tqdm(data_loader):
        bbox_label =  batch_data['gt_track_bbox'][0]
        estimation_boxes = batch_data['track_bbox'][0]
        loss_center = F.smooth_l1_loss(estimation_boxes[:, :3], bbox_label[:, :3])
        loss.append(loss_center)
    print(len(loss))
    print('finish')
